...
以前まではChromeウェブストアからuBlock Originをインストールできましたが、2026年8月をもってGoogleはManifest V2拡張機能をストアから削除しました。現在、独自のストアを持たずChromeウェブストアに依存するChromiumブラウザでuBlock Originをインストールするには、GitHubから手動でインストールする必要があります。
